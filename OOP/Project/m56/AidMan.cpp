		...
		while (valid) {
					...
					else
					{
						...
						if (choice == 1) {
							...
							else
							{
								...
								int amount;
								amount = ut.getint(1, m_ptr[search(skuTemp)]->qtyNeeded());
								m_ptr[search(skuTemp)]->operator+=(amount);
								std::cout << amount << " items added!\n";
							}
						}
						else if (choice == 2)
						{
							...
							else
							{
								...
								int amount;
								amount = ut.getint(1, m_ptr[search(skuTemp)]->qty());
								m_ptr[search(skuTemp)]->operator-=(amount);
								std::cout << amount << " items removed!\n";
							}
						}
						else
						{
			...
			if (choose == 5)
			{
				...
				while (sorted == true)
				{
					...
					{
						if ((m_ptr[i]->qty() - m_ptr[i]->qtyNeeded()) > (m_ptr[i + 1]->qty() - m_ptr[i + 1]->qtyNeeded()))
						{
							auto ptr = m_ptr[i]; // Buffer save prevQty data
							m_ptr[i] = m_ptr[i + 1]; //swap nextQty to prevQty
							m_ptr[i + 1] = ptr; //swap preQty to nextQty
			...
			{
				std::cout << "\n****" << "Ship Items" << "****\n";
				std::ofstream fp("shippingOrder.txt");
				int count = 0;
				int day, month, year;
				ut.getSystemDate(&year, &month, &day);
				fp << "Shipping Order, Date: " << year << "/" << std::setw(2) << std::setfill('0') << month << "/" << std::setw(2) << std::setfill('0') << day << "\n" << std::setfill(' ');
				fp << " ROW |  SKU  | Description                         | Have | Need |  Price  | Expiry\n";
				fp << "-----+-------+-------------------------------------+------+------+---------+-----------\n";
				...
				for (int i = 0; i < num_items; i++)
				{
					...
					{
						m_ptr[i]->linear(true);
						fp << std::setfill(' ') << std::setw(4) << count + 1;
						fp << " | ";
						m_ptr[i]->display(fp);
						count += 1;
						fp << "\n";
				...
				fp << "-----+-------+-------------------------------------+------+------+---------+-----------\n";
				std::cout << "Shipping Order for " << count << " times saved!\n\n";
				fp.close();
			}
			if (choose == 7)
			{
